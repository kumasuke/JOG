...
- **[P2] 複数 AIMU / NCVE ルール → 修正済み（Issue #53）**: 旧実装は同一キー/アップロードに複数ルールが重なる場合、prefix 一致の**最初のルールのみ**適用していた（過少削除＝安全側だが S3 非準拠）。S3 は overlapping ルールを独立評価し**最短の失効を採用**する（lifecycle-conflicts.md「if two expiration policies overlap, the shorter expiration policy is honored」、Example 3 で空 prefix 10日 + `logs/` 365日 → 10日が勝つ）。これを**フィールド合算ではなく verdict レベルの union** で実装: NCVE は `noncurrentRules` で全マッチルールを返し、各ルールごとに `noncurrentExpiryCandidates` を呼んで返る VersionID を dedup（設定順・rule 内は oldest-first を維持）して union。AIMU は `abortMPURules` で全ルールを返し、各アップロードを `aimuUploadEligible`（いずれかのルールの prefix 一致 かつ days 経過なら true）で判定。フィールド合算（最小 keep と最小 days を 1 ルールに合成）は**どの単一ルールでも消えないバージョンを消す過剰削除**になるため採らない。`NewerNoncurrentVersions` の保護件数は各ルールが自分の keep で全実非現行版を数える既存仕様を維持（[P1/2巡目] と整合）。ユニットテスト（`noncurrentRules`/`abortMPURules`/`aimuUploadEligible`）＋エンジン E2E（`TestEngine_NoncurrentVersionExpiration_OverlappingRules`：broad ルール単独なら全保護だが union で削除／`TestEngine_AbortIncompleteMultipartUpload_OverlappingRules`：disjoint prefix の両ルール評価）で固定。
  - **スコープ外（別 Issue 候補）**: current オブジェクトの `Expiration`（`currentExpirationRule`）も first-match で「最早 Days が勝つ」を満たさないが、Issue #53 は NCVE/AIMU 名指しのため本 PR では変更しない。
