...
3. **EODM の自動掃除（Expiration.Days 併用時の S3 付随挙動）と ExpiredObjectDeleteMarker+Days 同時指定の PUT バリデーション**は v1 対象外。後者は現状 PUT が受理してしまう（S3 は拒否）ので別 Issue 候補。
4. **通知イベント**: S3 は lifecycle 削除で `LifecycleExpiration:*` イベントを発行するが、JOG は通知配信自体が未実装（TODO.md 2.5）。エンジンの Action 発火点にフックを置ける構造（Report 経由）にして将来対応。
