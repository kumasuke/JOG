...
## 4. 長時間連続稼働・大量データのソーク（重い・任意）

> 具体的な手順（ミニソーク 30〜60分／24h 本格ソーク）と再利用可能な投入ツールは **[`LIFECYCLE_SOAK.md`](./LIFECYCLE_SOAK.md)** に分離した。以下は概要。

エンジン稼働下でリクエストパスのレイテンシ影響・リーク・安定性を実環境で測る、最も重い試験。数百万オブジェクトの作成にディスク・時間を大きく消費する。
